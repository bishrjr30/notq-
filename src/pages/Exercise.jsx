...
import { Exercise, Student, Recording } from "@/api/entities"; // ✅ التعامل مع Supabase عبر entities
import { supabase } from "@/api/supabaseClient";               // ✅ رفع التسجيلات إلى Supabase Storage
import { InvokeLLM } from "@/api/aiclient";                    // ✅ استخدام الذكاء الاصطناعي للأسئلة
...
// ✅ دالة رفع التسجيل إلى Supabase Storage (bucket اسمه "recordings")
async function uploadRecordingToSupabase(file, studentId) {
  const ext = file.name.split(".").pop() || "webm";
  const filePath = `recordings/${studentId}/${Date.now()}.${ext}`;
  ...
  if (error) {
    ...
    throw new Error("فشل في رفع الملف الصوتي إلى التخزين.");
  }
...
export default function ExercisePage() {
  ...
  const loadExercise = async () => {
    try {
      ...
      if (exerciseId) {
        const exerciseData = await Exercise.get(exerciseId); // ✅ من Supabase
        setExercise(exerciseData);
      }

      if (studentId) {
        const studentData = await Student.get(studentId); // ✅ من Supabase
  ...
  const submitRecording = async () => {
    ...
    if (!OPENAI_API_KEY) {
      setError(
        "لم يتم إعداد مفتاح OpenAI. أضِف VITE_OPENAI_API_KEY في الإعدادات (Vercel أو env) ثم أعد المحاولة."
      );
      return;
    }
    ...
    try {
      ...
      if (fileSizeKB < 2) {
        setError(
          "التسجيل فارغ أو قصير جداً. يرجى التأكد من التحدث بوضوح لمدة كافية."
        );
        setIsSending(false);
      ...
      const fileName = `recording_${student.name || student.id}_${timestamp}.webm`;
      ...
      const file_url = await uploadRecordingToSupabase(file, student.id);

      setAnalysisProgress(40);

      // ✅ 2) تحويل الصوت إلى نص (Whisper)
      ...
      if (!transcriptionResponse.ok) {
        ...
        if (
          ...
          throw new Error(
            "⚠️ عذراً، تم تجاوز حد استخدام الذكاء الاصطناعي. يرجى من المعلم استخدام مفتاح API بخطة صالحة."
          );
        }
      ...
      if (!analysisResponse.ok) {
        ...
        if (
          ...
          throw new Error(
            "⚠️ عذراً، تم تجاوز حد استخدام الذكاء الاصطناعي. يرجى من المعلم إضافة مفتاح API بخطة صالحة."
          );
        }
        throw new Error(`خطأ في خدمة التحليل (GPT-4o): ${errText}`);
      }
      ...
      // ✅ 4) حفظ التسجيل والتحليل في Supabase
      const recordingData = {
      ...
      if (
        ...
        errorMessage =
          "عذراً، وصل النظام إلى الحد الأقصى للاستخدام الشهري. يرجى إبلاغ المعلم بذلك.";
                              ...
                              <div className="mt-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 border-2 border-blue-200">
                                <div className="flex items-center justify-center gap-2 mb-2">
                                  ...
                                  <p className="font-bold text-blue-900 arabic-text">
                                    تقييم متقدم بالذكاء الاصطناعي
                                  </p>
                                </div>
                                <p className="text-sm text-blue-700 arabic-text">
                                  سيتم تحليل تسجيلك بدقة وإعطاؤك ملاحظات
                                  مفصلة لتحسين النطق
                                </p>
                              </div>
                            </div>
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center justify-center gap-2">
                                  ...
                                  <p className="text-indigo-700 arabic-text font-semibold">
                                    جاري تحليل الصوت باستخدام الذكاء
                                    الاصطناعي...
                                  </p>
                                </div>
                            ...
                            >
                              ...
                              <div className="bg-yellow-50 p-4 rounded-lg border-2 border-yellow-200 mb-4">
                                ...
                                {lastAnalysis.analysis_details?.suggestions && (
                                  <p className="text-yellow-700 arabic-text mt-2 pt-2 border-top border-yellow-200">
                              ...
                              <p className="text-xs text-gray-400 text-center mt-4 arabic-text">
                                تم التحليل باستخدام نماذج OpenAI المتقدمة
                              </p>
                            </motion.div>
                          )}
                          <p className="text-xl text-indigo-700 arabic-text leading-relaxed">
                            تسجيلك محفوظ، ويمكنك متابعة التمرين أو حل أسئلة
                            الفهم إن توفّرت.
                          </p>
